...
function init_MoviePage() {
    ...
    $('.score').raty({
        ...
        click:function (score, evt) {
            ...
            console.log(data);
            $.post("/score",data,function (result) {

                if(result=="评分成功！"){
                    alert("评分成功！");
                    $("#score").html('<span class="rates__done">感谢您的评分<span>');
                }
                else if(result=="您已对该电影评过分！"){
                    alert("您已经对该电影评过分");
                    $("#score").html('<span class="rates__done">您已评过分<span>');
                }
            });
            // if(str=="评分成功！"){
            ...
            // };
            // if(str=="您已对该电影评过分！"){
            //     $(this).html('<span class="rates__done">您已评过分<span>');
            // }
        }
...
function init_Rates() {
    "use strict";
    var scoreNodes = document.getElementsByClassName("score");
    for(var i=0;i<scoreNodes.length;i++){
        var score = scoreNodes[i].getAttribute("value");
        $('#'+i).raty({
            width: 130,
            score: score,
            path: '/images/rate/',
